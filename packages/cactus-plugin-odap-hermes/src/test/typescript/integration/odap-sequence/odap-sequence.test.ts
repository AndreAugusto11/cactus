...
const MAX_GATEWAYS = 25;

let totalNumberOfAssets = 0;

for (let i = MAX_GATEWAYS; i >= 0; i--) {
  totalNumberOfAssets += i;
}

const FABRIC_ASSET_IDS: string[] = new Array(totalNumberOfAssets);
const BESU_ASSET_IDS: string[] = new Array(totalNumberOfAssets);

for (let i = 0; i <= totalNumberOfAssets; i++) {
  FABRIC_ASSET_IDS[i] = uuidv4();
  BESU_ASSET_IDS[i] = uuidv4();
}
...
beforeAll(async () => {
  ...
  }
  {
    ...
    const response = await fabricApiClient.deployContractV1({
      channelId,
      ccVersion: "1.0.0",
      sourceFiles,
    ...
    for (let i = 0; i <= totalNumberOfAssets; i++) {
      ...
      log.info(`number: ${i}/${totalNumberOfAssets}`);
    }
  }
  {
...
test("runs ODAP between two gateways via openApi", async () => {
  fs.writeFileSync(path.join(__dirname, "odap-sequence.txt"), "INIT: 0\n");

  let count = 0;
  for (let i = 0; i < MAX_GATEWAYS; i++) {
    ...
    for (let j = 0; j < i; j++) {
      {
        ...
        const odapClientGatewayPluginOptions: IPluginOdapGatewayConstructorOptions = {
          ...
          fabricAssetID: FABRIC_ASSET_IDS[count],
        ...
        const odapServerGatewayPluginOptions: IPluginOdapGatewayConstructorOptions = {
          ...
          instanceId: uuidv4(),
          ipfsPath: ipfsApiHost,
          besuAssetID: BESU_ASSET_IDS[count++],
          besuPath: besuPath,
        ...
        pluginSourceGateway = new PluginOdapGateway(
    ...
    fs.appendFileSync(
      path.join(__dirname, "odap-sequence.txt"),
      "Run number " +
        (i + 1).toString() +
        ": " +
        Number(Date.now() - timer).toString() +
        "\n",
    );
  }
});
