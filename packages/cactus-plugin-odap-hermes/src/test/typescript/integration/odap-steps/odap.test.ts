...
const NUMBER_ITERATIONS = 5;

const FABRIC_ASSET_IDS = [uuidv4(), uuidv4(), uuidv4(), uuidv4(), uuidv4()];
const BESU_ASSET_IDS = [uuidv4(), uuidv4(), uuidv4(), uuidv4(), uuidv4()];
...
test("runs ODAP between two gateways via openApi", async () => {
  fs.writeFileSync(path.join(__dirname, "odap-1-1.txt"), "INIT: 0\n");
  ...
  for (let i = 0; i < NUMBER_ITERATIONS; i++) {
    ...
    fs.appendFileSync(
      ...
      "BEFORE RUN:" + Number(Date.now() - time).toString() + "\n",
    );

    const sessionID = pluginSourceGateway.configureOdapSession(
      odapClientRequest,
    );
    ...
    fs.appendFileSync(
      ...
      "P1:" + Number(Date.now() - time).toString() + "\n",
    );
    time = Date.now();
    ...
    fs.appendFileSync(
      "packages/cactus-plugin-odap-hermes/src/test/typescript/integration/odap-steps/odap-1-1.txt",
      "P2:" + Number(Date.now() - time).toString() + "\n",
    );
    time = Date.now();
    ...
    await checkValidCommitFinalResponse(
      commitFinalResponse,
      pluginSourceGateway,
    );
    ...
    fs.appendFileSync(
      "packages/cactus-plugin-odap-hermes/src/test/typescript/integration/odap-steps/odap-1-1.txt",
      "P3:" + Number(Date.now() - time).toString() + "\n",
    );
