...
beforeAll(async () => {
...

  {
    ...
    fs.appendFileSync(
      ...
      "IPFS:" + Number(Date.now() - timer).toString() + "\n",
    );
    timer = Date.now();
  }
  {
    ...
    fs.appendFileSync(
      path.join(__dirname, "odap-1-1.txt"),
      "FABRIC:" + Number(Date.now() - timer).toString() + "\n",
    );
    timer = Date.now();
  }
  {
    ...
    fs.appendFileSync(
      path.join(__dirname, "odap-1-1.txt"),
      "BESU:" + Number(Date.now() - timer).toString() + "\n",
    );
    timer = Date.now();
  }
  {
  ...
  fs.appendFileSync(
    path.join(__dirname, "odap-1-1.txt"),
    "GATEWAYS:" + Number(Date.now() - timer).toString() + "\n",
  );
  timer = Date.now();
});
