...
beforeAll(async () => {
...

  {
    ...
    fs.appendFileSync(
      ...
      "AFTER IPFS:" + Number(Date.now() - timer).toString() + "\n",
    );
  }
  {
    ...
    fs.appendFileSync(
      path.join(__dirname, "odap-1-1.txt"),
      "AFTER FABRIC:" + Number(Date.now() - timer).toString() + "\n",
    );
  }
  {
    ...
    fs.appendFileSync(
      path.join(__dirname, "odap-1-1.txt"),
      "AFTER BESU:" + Number(Date.now() - timer).toString() + "\n",
    );
  }
  {
  ...
  fs.appendFileSync(
    path.join(__dirname, "odap-1-1.txt"),
    "AFTER GATEWAYS:" + Number(Date.now() - timer).toString() + "\n",
  );
});
...
test("runs ODAP between two gateways via openApi", async () => {
  ...
  fs.appendFileSync(
    path.join(__dirname, "odap-1-1.txt"),
    "BEFORE RUN:" + Number(Date.now() - timer).toString() + "\n",
  );
  const res = await apiClient.clientRequestV1(odapClientRequest);
  fs.appendFileSync(
    path.join(__dirname, "odap-1-1.txt"),
    "AFTER RUN:" + Number(Date.now() - timer).toString() + "\n",
  );
