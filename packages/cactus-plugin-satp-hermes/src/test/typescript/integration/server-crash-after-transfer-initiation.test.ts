...
let satpClientGatewayPluginOptions: IFabricSatpGatewayConstructorOptions;
let satpServerGatewayPluginOptions: IBesuSatpGatewayConstructorOptions;
...
let satpServerGatewayApiHost: string;
let satpClientGatewayApiHost: string;

let satpClientRequest: ClientV1Request;
...
beforeAll(async () => {
  ...
  }
  {
    ...
    satpServerGatewayPluginOptions = {
    ...
    const { address, port } = addressInfo;
    satpServerGatewayApiHost = `http://${address}:${port}`;
  ...
  }
  {
    ...
    satpClientGatewayPluginOptions = {
      name: "cactus-plugin#satpGateway",
      dltIDs: ["DLT2"],
      instanceId: uuidv4(),
      ipfsPath: ipfsApiHost,
    ...
    const { address, port } = addressInfo;
    satpClientGatewayApiHost = `http://${address}:${port}`;

    pluginSourceGateway = new FabricSatpGateway(satpClientGatewayPluginOptions);
    ...
    satpClientRequest = {
      clientGatewayConfiguration: {
        apiHost: satpClientGatewayApiHost,
      },
      serverGatewayConfiguration: {
        apiHost: satpServerGatewayApiHost,
      },
      version: "0.0.0",
...
test("server gateway crashes after transfer initiation flow", async () => {
  const sessionID = pluginSourceGateway.configureSatpSession(satpClientRequest);
  ...
  pluginRecipientGateway = new BesuSatpGateway(satpServerGatewayPluginOptions);
  await pluginRecipientGateway.registerWebServices(serverExpressApp);
