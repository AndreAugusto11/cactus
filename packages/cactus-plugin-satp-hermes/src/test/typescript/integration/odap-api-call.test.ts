...
test("runs ODAP between two gateways via openApi", async () => {
  const satpClientGatewayPluginOptions: IPluginSatpGatewayConstructorOptions = {
  ...
  const satpServerGatewayPluginOptions: IPluginSatpGatewayConstructorOptions = {
    name: "cactus-plugin#satpGateway",
    dltIDs: ["DLT1"],
    instanceId: uuidv4(),
    ipfsPath: ipfsApiHost,
  ...
  }
  {
    ...
    const { address, port } = addressInfo;
    const satpClientGatewayApiHost = `http://${address}:${port}`;
    ...
    const satpApiConfig = new Configuration({
      basePath: satpClientGatewayApiHost,
    });
    const apiClient = new OdapApi(satpApiConfig);

    const expiryDate = new Date(2060, 11, 24).toString();
    const assetProfile: AssetProfile = { expirationDate: expiryDate };

    const satpClientRequest: ClientV1Request = {
    ...
    const res = await apiClient.clientRequestV1(satpClientRequest);
