...
test("check valid built of recover update message", async () => {
  ...
  await pluginSourceGateway.storeSatpLog(satpLog1);

  const satpLog2: OdapLocalLog = {
    sessionID: sessionID,
    type: "exec",
    operation: "validate",
    data: JSON.stringify(sessionData),
  ...
  await pluginRecipientGateway.storeSatpLog(satpLog2);

  const satpLog3: OdapLocalLog = {
    sessionID: sessionID,
    type: "done",
    operation: "validate",
    data: JSON.stringify(sessionData),
  ...
  await pluginRecipientGateway.storeSatpLog(satpLog3);

  const satpLog4: OdapLocalLog = {
    sessionID: sessionID,
    type: "ack",
    operation: "validate",
    data: JSON.stringify(sessionData),
  ...
  await pluginRecipientGateway.storeSatpLog(satpLog4);
