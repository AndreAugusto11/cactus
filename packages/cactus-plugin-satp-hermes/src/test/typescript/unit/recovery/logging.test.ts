...
let satpLog: OdapLocalLog;
let satpLog2: OdapLocalLog;
let satpLog3: OdapLocalLog;
let satpLog4: OdapLocalLog;
...
test("successful logging of proof to ipfs and sqlite", async () => {
  const claim = "claim";
  const satpLogKey = PluginSatpGateway.getOdapLogKey(
    sessionID,
    "proof",
    "lock",
  );

  await pluginSourceGateway.storeSatpProof({
    sessionID,
    type: "proof",
    operation: "lock",
    data: claim,
  ...
  const retrievedLogIPFS = await pluginSourceGateway.getLogFromIPFS(satpLogKey);
  ...
  expect(retrievedLogIPFS.key).toBe(satpLogKey);
...
test("successful logging to ipfs and sqlite", async () => {
  ...
  await pluginSourceGateway.storeSatpLog(satpLog);
...
test("successful retrieval of last log", async () => {
  ...
  await pluginSourceGateway.storeSatpLog(satpLog2);
  await pluginSourceGateway.storeSatpLog(satpLog3);
...
test("successful recover of sessions after crash", async () => {
  ...
  await pluginSourceGateway.storeSatpLog(satpLog3);
  await pluginSourceGateway.storeSatpLog(satpLog2);
  await pluginSourceGateway.storeSatpLog(satpLog4);
